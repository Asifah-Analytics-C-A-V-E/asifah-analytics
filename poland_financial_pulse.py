...
# ── SHARED MARKET FETCHER (v1.0, Jul 12 2026) ────────────────────────────
# Yahoo was 429-ing EVERY ticker from the Render IP -- including BZ=F, the same
# Brent symbol russia_stability has fetched for months. Not a pacing problem and
# not a symbol problem: the datacenter IP is bot-flagged.
# market_fetch.py fixes it with curl_cffi TLS impersonation (already approved on
# this platform for RSS bot detection) and falls back to Stooq, a keyless Polish
# CSV feed. Soft import: if it's missing we keep the legacy local fetchers.
try:
    from market_fetch import fetch_chain as _mf_chain, diagnostics as _mf_diag
    MARKET_FETCH_AVAILABLE = True
except ImportError:
    MARKET_FETCH_AVAILABLE = False
    _mf_diag = lambda: {'market_fetch': 'not installed'}
...
def _fetch_chain(candidates):
    # Prefer the shared fetcher: curl_cffi (Chrome TLS) -> Stooq -> requests.
    # The legacy local chain below is kept as a fallback if market_fetch.py
    # isn't deployed yet.
    if MARKET_FETCH_AVAILABLE:
        out = _mf_chain(candidates, log='[PL Financial]')
        if out is None:
            from market_fetch import THROTTLED as _MF_THROTTLED, LAST_ERRORS as _MF_ERRS
            _THROTTLED.update(_MF_THROTTLED & set(candidates))
            _LAST_ERRORS.update({k: v for k, v in _MF_ERRS.items() if k in candidates})
        return out
    return _fetch_chain_legacy(candidates)


def _fetch_chain_legacy(candidates):
...
def register_poland_financial_endpoints(app):
    ...
    def debug_poland_financial():
        ...
        return jsonify({
            ...
            'fetcher':          (_mf_diag() if MARKET_FETCH_AVAILABLE
                                 else {'market_fetch': 'NOT INSTALLED -- upload market_fetch.py'}),
