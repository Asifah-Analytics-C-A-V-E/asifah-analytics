...
def _fetch_with_failover(primary, fallback=None):
    if MARKET_FETCH_AVAILABLE:
        ...
        out = _mf_chain(chain, log='[KZ Financial]')
        if out is None:
            from market_fetch import THROTTLED as _MF_T, LAST_ERRORS as _MF_E
            _THROTTLED.update(_MF_T & set(chain))
            _LAST_ERRORS.update({k: v for k, v in _MF_E.items() if k in chain})
        return out
    return _fetch_with_failover_legacy(primary, fallback)


def _fetch_with_failover_legacy(primary, fallback=None):
...
def get_kazakhstan_financial(force=False):
    ...
    brent = _fetch_with_failover('BZ=F')
    kzt   = _fetch_with_failover('KZT=X')
